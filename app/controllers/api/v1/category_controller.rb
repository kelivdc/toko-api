class Api::V1::CategoryController < ApplicationController
  def index
    if params[:id]
      record = Category.where(id: params[:id]).order(name: :asc)
      ...
      # per_page = 15 if params[:id]
      page = (start_param / per_page) + 1
      q = Category.ransack(name_or_slug_cont: params[:q])
  ...
  def update
    if record = Category.find(params[:id])
  ...
  def create
    if record = Category.create(record_params)
      render json: record
    else
